...
import { getUserImageOrDefault } from "./../utils";
import Link from "./Link";

function view(vnode) {
  var currentUser = vnode.attrs.currentUser
    ? vnode.attrs.currentUser
    : {
        username: "",
      };
  ...
  var allLinks = {
    ...
    editor: {
      route: "/editor",
      label: '<i class="ion-compose"></i> New Article',
    },
    settings: {
      route: "/settings",
      label: '<i class="ion-gear-a"></i> Settings',
    },
    user: {
      route: "/@" + currentUser.username,
      label:
        '<img class="user-pic" src="' +
        getUserImageOrDefault(currentUser) +
        '" /> ' +
        currentUser.username,
    },
  };

  var linksForGuest = [allLinks.home, allLinks.login, allLinks.register];
  ...
  return m(
    ...
    linksToDisplay.map(function (link) {
      ...
      return m(
        "li.nav-item",
        m(Link, { className: className, to: link.route }, m.trust(link.label))
      );
    })
  );
}
